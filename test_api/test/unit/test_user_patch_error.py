import pytest
from test_api.clients.api_httpx_unit import async_response_patch # 發出 PUT 請求
...
async def test_success_user_id(default_user, user_data, test_session, client):
    ...
    response = await async_response_patch(user_id, user_data, client)
...
async def test_404_user_id(default_user, user_id, client):
    response = await async_response_patch(user_id, {}, client)
...
async def test_update_user_fail_422_due_to_short_fields(default_user, username_password, expected_error_key, client):
    user_id = default_user["id"]
    response = await async_response_patch(user_id, username_password, client)
...
async def test_patch_user_fail_403_due_to_id_field(default_user, id, expected_error_key, client):
    user_id = default_user["id"]
    response = await async_response_patch(user_id, id, client)
...
async def test_patch_user_fail_400_due_to_id_field(default_user, id, expected_error_key, client):
    user_id = default_user["id"]
    response = await async_response_patch(user_id, id, client)
