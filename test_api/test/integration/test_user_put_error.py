import pytest
from test_api.clients.api_httpx_integration import async_response_put  # 發出 PUT 請求
...
async def test_success_user_id(default_user, user_data, get_test_session):
    ...
    response = await async_response_put(user_id, user_data)
...
async def test_404_user_id(default_user, user_id):
    response = await async_response_put(user_id, {"username": "new_eric", "email": "new_eric@example.com", "password": "newpassword123"})
...
async def test_update_user_fail_422_due_to_short_fields(default_user, username_password, expected_error_key):
    user_id = default_user["id"]
    response = await async_response_put(user_id, username_password)
...
async def test_put_fail_422_due_to_id_field(default_user, id, expected_error_key):
    user_id = default_user["id"]
    response = await async_response_put(user_id, id)
    await assert_response_fail_422(response, expected_error_key)